ARG BASE_IMAGE=provarepro/tensorflow:deps-py38-gcc93-ubuntu20
...
#    bazel build -j 4 ${BAZEL_ARGS[@]} //tensorflow/tools/pip_package:build_pip_package && \
...
#RUN cd tensorflow-${TF_VER} && \
#    bazel build -j 4 --config=mkl --config=opt --config=monolithic -c opt //tensorflow:libtensorflow_cc.so
#
...
#COPY py38-gcc93-ubuntu20 /tmp/tf_files
