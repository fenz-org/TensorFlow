...
RUN python${PYTHON_VERSION} -m pip install keras_preprocessing && \
    cd tensorflow-${TF_VER} && \
    bazel build --local_ram_resources=2048 --jobs 2 --config=mkl --config=opt -c opt //tensorflow/tools/pip_package:build_pip_package ^^ \
